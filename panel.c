...
void NP_update(REGS *regs)
{
    ...
    if (NPcpugraph)
    {
        for (i = 0; i < sysblk.numcpu; i++)
        {
            ...
            {
                n = 33 * sysblk.regs[i]->cpupct;
                if (n <= 0 && sysblk.regs[i]->cpupct > 0.00)
                    n = 1;
                else if (n > 33)
                    n = 33;
                set_color (n > 16 ? COLOR_WHITE : COLOR_LIGHT_GREY, COLOR_BLACK);
                set_pos (26+i, 5);
                if (n > 0) fill_text ('*', n+5);
                fill_text (' ', 38);
        ...
        */
        ...
        if (NPDup == 0) {
            ...
            if (redraw_status && !sysblk.npquiet)
            {
                ...
                {
                    sprintf (buf, "instcount=%llu",
#if defined(_FEATURE_SIE)
                        SIE_MODE(regs) ?  (long long) regs->hostregs->instcount :
#endif /*defined(_FEATURE_SIE)*/
                        (long long)regs->instcount);
                    set_pos (cons_rows, cons_cols - strlen(buf));
                    draw_text (buf);
                    set_pos (cons_rows, 1);
                    sprintf (buf, "CPU%4.4X PSW=%8.8X %8.8X",
                             regs->cpuad, fetch_fw (curpsw), fetch_fw(curpsw+4));
                    draw_text (buf); 
                    if (regs->arch_mode == ARCH_900)
                    {
                        draw_char (' ');
                    ...
                    draw_char (' ');
                    draw_char (regs->cpustate == CPUSTATE_STOPPED ? 'M' : '.');
                    draw_char (sysblk.inststep                    ? 'T' : '.');
                    draw_char (WAITSTATE(&regs->psw)              ? 'W' : '.');
                    draw_char (regs->loadstate                    ? 'L' : '.');
                    draw_char (regs->checkstop                    ? 'C' : '.');
                    draw_char (PROBSTATE(&regs->psw)              ? 'P' : '.');
                    draw_char (SIE_MODE(regs)                     ? 'S' : '.');
                    draw_char (regs->arch_mode == ARCH_900        ? 'Z' : '.');
                    draw_char (' ');
                }
                else
                {
                    set_pos (cons_rows, 1);
                    sprintf (buf,"CPU%4.4X Offline ", sysblk.pcpu);
                    draw_text (buf);
                }
