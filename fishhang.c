////////////////////////////////////////////////////////////////////////////////////
//         fishhang.c           determine where fthreads is hanging
////////////////////////////////////////////////////////////////////////////////////

// (file included as part of w32chan.c)

/////////////////////////////////////////////////////////////////////////////

BOOL bFishHangAtExit = FALSE; void FishHangAtExit() {bFishHangAtExit = TRUE;}

LIST_ENTRY  ThreadsListHead;	// head list entry of list of threads;
LIST_ENTRY  LocksListHead;		// head list entry of list of locks;
LIST_ENTRY  EventsListHead;		// head list entry of list of locks;

CRITICAL_SECTION  ListsLock;	// (lock for manipulating above lists)
...
)
{
	...
	while (pListEntry != &ThreadsListHead)
	{
		...
		for (;;)
		{
			...
			if (!pOwningThread ||
				!pOwningThread->bWaitingForLock) break;
...
)
{
	...
	GetSystemTime(&pFISH_THREAD->timeWaiting);
	bDeadlockDetected = PrintDeadlock(pFISH_THREAD,pszFileWaiting,nLineWaiting);
	UnlockFishHang();

	if (bDeadlockDetected)
	{
		bFishHangAtExit = TRUE;
		Sleep(100);  // (give system time to display the deadlock report)
		exit(-1);
	}
...
)
{
	...
	bCriticalSectionAcquired = TryEnterCriticalSection(lpCriticalSection);

	LockFishHang();
	pFISH_THREAD->bWaitingForLock = FALSE;
    if (bCriticalSectionAcquired)
    {
	    if (pFISH_LOCK->pOwningThread != pFISH_THREAD)
	    {
		    pFISH_LOCK->pOwningThread = pFISH_THREAD;
    ...
    return bCriticalSectionAcquired;
}
...
)
{
	...
	if (!GetThreadAndLockPtrs(&pFISH_THREAD,&pFISH_LOCK,lpCriticalSection))
		FishHangAbort(__FILE__,__LINE__);
