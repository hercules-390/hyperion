...
static int ecpsvm_disp_incprobt(REGS *regs,VADR vmb)
{
    ...
    MSGBUF(buf, "INCPROBT Entry : VMTMOUTQ = %16.16"PRIx64,DW_VMTMOUTQ);
    DEBUG_CPASSISTX(DISP0,WRMSG(HHC90000, "D", buf));
    MSGBUF(buf, "INCPROBT Entry : PROBSTRT = %16.16"PRIx64,DW_PROBSTRT);
    DEBUG_CPASSISTX(DISP0,WRMSG(HHC90000, "D", buf));
    ...
    MSGBUF(buf, "INCPROBT TSPENT = %16.16"PRIx64,tspent);
    DEBUG_CPASSISTX(DISP0,WRMSG(HHC90000, "D", buf));
    DW_PROBTIME=EVM_LD(PROBTIME);
    DW_PROBTIME-=tspent;
    EVM_STD(DW_PROBTIME,PROBTIME);
    MSGBUF(buf, "INCPROBT NEW PROBTIME = %16.16"PRIx64,DW_PROBTIME);
    DEBUG_CPASSISTX(DISP0,WRMSG(HHC90000, "D", buf));
    return(2);
}
