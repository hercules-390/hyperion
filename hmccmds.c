...
int quiet_cmd(char* cmdline, int argc, char *argv[])
{
    ...
    if (extgui)
    {
        logmsg( _("Ignored. (external GUI active)\n") );
        return 0;
    }
#endif /*EXTERNALGUI*/
    NPquiet = !NPquiet;
    logmsg( _("Automatic refresh %s.\n"), NPquiet ? _("disabled") : _("enabled") );
    return 0;
}
...
int syncio_cmd(char* cmdline, int argc, char *argv[])
{
    ...
    if (!found)
        logmsg( _("No synchronous I/O devices found\n") );
    else
...
int icount_cmd(char* cmdline, int argc, char *argv[])
{
    ...
    if (argc > 1 && !strcasecmp(argv[1], "clear"))
    {
        ...
        logmsg( _("Instruction counts reset to zero.\n") );
        return 0;
    }
...
int ProcessPanelCommand (const char* pszCmdLine)
{
    ...
    logmsg( _("Command \"%s\" not found; enter '?' for list.\n"), cmd_argv[0] );
...
int ListAllCommands(char* cmdline, int argc, char *argv[])
{
    ...
    logmsg( _("Valid panel commands are...\n \n") );
    logmsg( "  %-9.9s    %s \n \n", "Command", "Description..." );
    logmsg( "  %-9.9s    %s \n \n", "-------", "-----------------------------------------------" );
...
int HelpCommand(char* cmdline, int argc, char *argv[])
{
    ...
    logmsg( _("No additional help available.\n") );
    return -1;
}
